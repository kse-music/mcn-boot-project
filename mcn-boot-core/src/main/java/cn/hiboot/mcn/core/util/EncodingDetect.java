 ...
 *
 * @author DingHao
 * @since 2024/7/11 9:58
 */
final class EncodingDetect extends Encoding {

    private final static Logger log = LoggerFactory.getLogger(EncodingDetect.class);
    ...
    public int detectEncoding(URL testurl) {
        ...
        try {
            ...
            log.error("Error loading or using URL {}", e.getMessage());
            guess = -1;
        }
        return guess;
    }
    ...
    public String getEncoding(byte[] rawtext) {
        return javaname[detectEncoding(rawtext)];
    }
    ...
    public int detectEncoding(byte[] rawtext) {
        ...
        for (index = 0; index < TOTALTYPES; index++) {
            if (debug)
                log.error("Encoding {} score {}", nicename[index], scores[index]);
