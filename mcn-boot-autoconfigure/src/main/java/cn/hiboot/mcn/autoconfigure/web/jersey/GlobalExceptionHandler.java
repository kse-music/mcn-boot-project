...
public class GlobalExceptionHandler extends AbstractExceptionHandler implements ExceptionMapper<Exception> {
    ...
    public Response toResponse(Exception e) {
        ...
        int errorCode = ExceptionKeys.SERVICE_ERROR;
        String errorInfo = "";
        Object data = null;
        if(e instanceof BaseException){
            errorCode = ((BaseException)e).getCode();
            errorInfo = ((BaseException)e).getMsg();
        }else if(e instanceof ParamException || e instanceof ValidationException){
            errorCode = ExceptionKeys.PARAM_PARSE_ERROR;
            data = parseParamException(e);
        }else if(e instanceof WebApplicationException){
            if (e instanceof NotFoundException) {
                statusCode = Response.Status.NOT_FOUND;
                errorCode = ExceptionKeys.HTTP_ERROR_404;
            } else if (e instanceof NotAllowedException) {
                statusCode = Response.Status.METHOD_NOT_ALLOWED;
                errorCode = ExceptionKeys.HTTP_ERROR_405;
            } else if (e instanceof NotAcceptableException) {
                statusCode = Response.Status.NOT_ACCEPTABLE;
                errorCode = ExceptionKeys.HTTP_ERROR_406;
            } else {
                statusCode = Response.Status.INTERNAL_SERVER_ERROR;
                errorCode = ExceptionKeys.HTTP_ERROR_500;
            }
        }
        return Response.ok(buildErrorMessage(errorCode,errorInfo,data,e)).status(statusCode).type(MediaType.APPLICATION_JSON_TYPE).build();
    }

    private Object parseParamException(Exception e) {
