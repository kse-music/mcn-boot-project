 ...
 */
class DefaultDbTypeManager implements DbTypeManager {

    private static final Map<String, DbTypeProvider> REGISTRY = new ConcurrentHashMap<>();

    static final DbTypeManager defaults = new DefaultDbTypeManager();

    private DefaultDbTypeManager() {
        registerDefault();
        ServiceLoader.load(DbTypeProvider.class).forEach(this::register);
    }
    ...
    @Override
    public void register(DbTypeProvider... providers) {
    ...
    @Override
    public DbTypeProvider get(String name) {
        return REGISTRY.get(name.toLowerCase());
    }

    @Override
    public Collection<DbTypeProvider> all() {
